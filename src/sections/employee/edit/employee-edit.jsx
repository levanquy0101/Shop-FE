...
import * as EmployeeService from '../../../services/api/EmloyeeService'
import { NameApp } from 'src/data';
import { UploadOneImage } from "src/firebase/UploadImage";

function EmployeeEdit() {
    document.title = `Sửa nhân viên | ${NameApp}`
    ...
    const getRoleNotIn = async () => {
        const response = await RoleService.getAllNotIn()
        console.log(response);
        setRoles(response)
    }

    const handleOpenEditPass = (event) => {
        setValue("password", "");
    ...
    const onSubmit = async (data) => {
        try {
            ...
            data.gender = gender;
            await EmployeeService.putEmployee(id, data)
            reset();
    ...
    return (
        <Container>
            <Typography variant="h4">Sửa nhân viên</Typography>
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <Grid container spacing={2} fullWidth>
                                    <Grid item xs={12} sm={6}>
                                        <TextField fullWidth value={employee.code} label="Mã nhân viên" {...register("code", {})} required />
                                    </Grid>
                                    <Grid item xs={12} sm={6}>
                                    ...
                                    <Grid item xs={12} sm={6}>
                                        <FormControl fullWidth>
                                            ...
                                            >
                                                ...
                                                <MenuItem value={0} onClick={() => setGender(0)}>Nam</MenuItem>
                                                <MenuItem value={1} onClick={() => setGender(1)}>Nữ</MenuItem>
                                                <MenuItem value={2} onClick={() => setGender(2)}>Khác</MenuItem>
                                            </Select>
                                        </FormControl>
                                    </Grid>
                                    <Grid item xs={12} sm={6}>
                                    ...
                                    <Grid item xs={12} sm={6}>
                                        <FormControl fullWidth>
                                            ...
                                            >
                                                ...
                                                {
                                                    roles.map((role) => (
                                                        <MenuItem key={role.id} value={JSON.stringify(role)} onClick={() => setRole(JSON.stringify(role))}>
                                    ...
                                    <Grid item xs={12} sm={6} sx={{display: "flex",position:"relative" ,alignItems:"center"}}>
                                        <TextField fullWidth label="Mật khẩu" {...register("password", {})} disabled={isDisabled}  InputLabelProps={{ shrink: !isDisabled }} />
                                        <IconButton onClick={handleOpenEditPass} sx={{position: "absolute", right: 0}}>
                                            <AiTwotoneEdit />
                                        </IconButton>
                                    </Grid>
                                </Grid>
